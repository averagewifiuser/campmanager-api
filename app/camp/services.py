...
from .models import Camp, Church, Category, CustomField, RegistrationLink, Registration, db
...
class CampService:
    ...
    def get_user_camps(self, user_id: str) -> List[Camp]:
        ...
        try:
            return Camp.query.filter_by(camp_manager_id=user_id).order_by(Camp.created_at.desc()).all()
        except SQLAlchemyError as e:
    ...
    def create_camp(self, camp_data: Dict[str, Any]) -> Optional[Camp]:
        ...
        try:
            ...
            required_fields = ['name', 'start_date', 'end_date', 'location', 'base_fee', 'capacity', 'registration_deadline', 'camp_manager_id']
            ...
            if registration_deadline.date() > start_date:
                raise ValueError("Registration deadline must be before or on start date")
            ...
            current_app.logger.info(f"New camp created: {new_camp.name} by {camp_data['camp_manager_id']}")
            return new_camp
    ...
    def update_camp(self, camp_id: str, update_data: Dict[str, Any]) -> Optional[Camp]:
        ...
        try:
            ...
            # Update fields
            updatable_fields = ['name', 'start_date', 'end_date', 'location', 'base_fee', 'capacity', 'description', 'registration_deadline', 'is_active']
            for field in updatable_fields:
                if field in update_data:
                    if field in ['base_fee'] and update_data[field] is not None:
                    ...
                    elif field in ['registration_deadline'] and update_data[field] is not None:
                        deadline = update_data[field]
                        if isinstance(deadline, str):
                            deadline = datetime.fromisoformat(deadline)
                        setattr(camp, field, deadline)
                    elif field in ['start_date', 'end_date'] and update_data[field] is not None:
                        ...
                        if update_data[field] is not None:
                            value = update_data[field].strip() if isinstance(update_data[field], str) else update_data[field]
    ...
    def get_camp_stats(self, camp_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            checked_in_count = sum(1 for reg in camp.registrations if reg.has_checked_in)
            
            # Calculate capacity percentage
            capacity_percentage = (total_registrations / camp.capacity * 100) if camp.capacity > 0 else 0
            
            # Calculate revenue
            total_revenue = sum(float(reg.total_amount) for reg in camp.registrations if reg.has_paid)
...
class ChurchService:
    ...
    def create_church(self, church_data: Dict[str, Any]) -> Optional[Church]:
        ...
        try:
            ...
            current_app.logger.info(f"New church created: {new_church.name} for camp {church_data['camp_id']}")
            return new_church
    ...
    def update_church(self, church_id: str, update_data: Dict[str, Any]) -> Optional[Church]:
...
class CategoryService:
    ...
    def get_camp_categories(self, camp_id: str) -> List[Category]:
        ...
        try:
            return Category.query.filter_by(camp_id=camp_id).order_by(Category.name).all()
        except SQLAlchemyError as e:
    ...
    def create_category(self, category_data: Dict[str, Any]) -> Optional[Category]:
        ...
        try:
            ...
            if discount_percentage and discount_amount:
                raise ValueError("Cannot set both discount percentage and discount amount")
            ...
            if existing_category:
                raise ValueError("A category with this name already exists in this camp")
            
            new_category = Category(
                name=category_data['name'].strip(),
                camp_id=category_data['camp_id'],
                discount_percentage=Decimal(str(discount_percentage)) if discount_percentage else 0,
            ...
            current_app.logger.info(f"New category created: {new_category.name} for camp {category_data['camp_id']}")
            return new_category
    ...
    def update_category(self, category_id: str, update_data: Dict[str, Any]) -> Optional[Category]:
        ...
        try:
            ...
            if 'discount_percentage' in update_data and 'discount_amount' in update_data:
                if update_data['discount_percentage'] and update_data['discount_amount']:
                    raise ValueError("Cannot set both discount percentage and discount amount")
            
            if 'discount_percentage' in update_data and update_data['discount_percentage'] is not None:
            ...
            for field in updatable_fields:
                if field in update_data:
                    if field in ['discount_percentage', 'discount_amount'] and update_data[field] is not None:
...
class CustomFieldService:
    ...
    def get_custom_field_by_id(self, field_id: str) -> Optional[CustomField]:
        ...
        try:
            ...
            current_app.logger.error(f"Database error in get_custom_field_by_id: {str(e)}")
            return None
    
    def get_camp_custom_fields(self, camp_id: str) -> List[CustomField]:
        """Get all custom fields for a camp"""
        try:
            return CustomField.query.filter_by(camp_id=camp_id).order_by(CustomField.order, CustomField.field_name).all()
        except SQLAlchemyError as e:
            current_app.logger.error(f"Database error in get_camp_custom_fields: {str(e)}")
            return []
        ...
        try:
            ...
            if field_data['field_type'] in ['dropdown', 'checkbox']:
                ...
                if not options or len(options) == 0:
                    raise ValueError("Options are required for dropdown and checkbox fields")
            ...
            if existing_field:
                raise ValueError("A custom field with this name already exists in this camp")
            ...
            current_app.logger.info(f"New custom field created: {new_field.field_name} for camp {field_data['camp_id']}")
            return new_field
            ...
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in create_custom_field: {str(e)}")
            raise Exception("Failed to create custom field")
    
    def update_custom_field(self, field_id: str, update_data: Dict[str, Any]) -> Optional[CustomField]:
        ...
        try:
            ...
            if 'field_name' in update_data:
                ...
                existing_field = CustomField.query.filter_by(
                    field_name=name,
                    camp_id=custom_field.camp_id
                ).filter(CustomField.id != field_id).first()
            ...
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in update_custom_field: {str(e)}")
    ...
    def delete_custom_field(self, field_id: str) -> bool:
        ...
        try:
            ...
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in delete_custom_field: {str(e)}")
...
class RegistrationLinkService:
    ...
    def get_registration_link_by_id(self, link_id: str) -> Optional[RegistrationLink]:
        ...
        try:
            ...
            current_app.logger.error(f"Database error in get_registration_link_by_id: {str(e)}")
            return None
    ...
    def get_registration_link_by_token(self, token: str) -> Optional[RegistrationLink]:
        ...
        try:
            ...
            current_app.logger.error(f"Database error in get_registration_link_by_token: {str(e)}")
            return None
    
    def get_camp_registration_links(self, camp_id: str) -> List[RegistrationLink]:
        """Get all registration links for a camp"""
        try:
            return RegistrationLink.query.filter_by(camp_id=camp_id).order_by(RegistrationLink.created_at.desc()).all()
        except SQLAlchemyError as e:
            current_app.logger.error(f"Database error in get_camp_registration_links: {str(e)}")
            return []
    
    def create_registration_link(self, link_data: Dict[str, Any]) -> Optional[RegistrationLink]:
        ...
        try:
            ...
            if not isinstance(link_data['allowed_categories'], list) or len(link_data['allowed_categories']) == 0:
            ...
            current_app.logger.info(f"New registration link created: {new_link.name} for camp {link_data['camp_id']}")
            return new_link
            ...
            db.session.rollback()
            current_app.logger.error(f"Database error in create_registration_link: {str(e)}")
            raise Exception("Failed to create registration link due to database error")
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in create_registration_link: {str(e)}")
    def update_registration_link(self, link_id: str, update_data: Dict[str, Any]) -> Optional[RegistrationLink]:
        ...
        try:
            ...
            # Update other fields
            updatable_fields = ['allowed_categories', 'expires_at', 'usage_limit', 'is_active']
            for field in updatable_fields:
                if field in update_data:
            ...
            db.session.rollback()
            current_app.logger.error(f"Database error in update_registration_link: {str(e)}")
            raise Exception("Failed to update registration link due to database error")
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in update_registration_link: {str(e)}")
    ...
    def delete_registration_link(self, link_id: str) -> bool:
        ...
        try:
            ...
            if len(link.registrations) > 0:
                raise ValueError("Cannot delete registration link with existing registrations")
            ...
            db.session.rollback()
            current_app.logger.error(f"Database error in delete_registration_link: {str(e)}")
            raise Exception("Failed to delete registration link due to database error")
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in delete_registration_link: {str(e)}")
    ...
    def toggle_registration_link(self, link_id: str) -> Optional[RegistrationLink]:
        ...
        try:
            ...
            db.session.rollback()
            current_app.logger.error(f"Database error in toggle_registration_link: {str(e)}")
            raise Exception("Failed to toggle registration link due to database error")
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in toggle_registration_link: {str(e)}")
...
class RegistrationService:
    ...
    def get_registration_by_id(self, registration_id: str) -> Optional[Registration]:
        ...
        try:
            ...
            current_app.logger.error(f"Database error in get_registration_by_id: {str(e)}")
            return None
    
    def get_camp_registrations(self, camp_id: str) -> List[Registration]:
        """Get all registrations for a camp"""
        try:
            return Registration.query.filter_by(camp_id=camp_id).order_by(Registration.registration_date.desc()).all()
        except SQLAlchemyError as e:
            current_app.logger.error(f"Database error in get_camp_registrations: {str(e)}")
            return []
    
    def get_registration_form(self, camp_id: str, link_token: str = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Get churches
            churches = Church.query.filter_by(camp_id=camp_id).order_by(Church.name).all()
            
            # Get custom fields
            custom_fields = CustomField.query.filter_by(camp_id=camp_id).order_by(CustomField.order, CustomField.field_name).all()
            
            # Get categories based on link type
            registration_link = None
            if link_token:
                registration_link = RegistrationLink.query.filter_by(link_token=link_token).first()
                ...
                # Get all categories
                categories = Category.query.filter_by(camp_id=camp_id).order_by(Category.name).all()
                link_type = 'general'
            ...
            return {
                ...
                'link_type': link_type,
                'registration_link': registration_link.to_dict() if registration_link else None
            }
    ...
    def create_registration(self, registration_data: Dict[str, Any], link_token: str = None) -> Optional[Registration]:
        """Create a new registration"""
        try:
            # Validate required fields
            required_fields = ['surname', 'last_name', 'age', 'phone_number', 'emergency_contact_name', 'emergency_contact_phone', 'church_id', 'category_id', 'camp_id']
            for field in required_fields:
                if field not in registration_data or registration_data[field] is None:
                    raise ValueError(f"Missing required field: {field}")
            
            # Get camp and validate
            camp = Camp.query.filter_by(id=registration_data['camp_id'], is_active=True).first()
            if not camp:
            ...
            # Check capacity
            current_registrations = Registration.query.filter_by(camp_id=camp.id).count()
            if current_registrations >= camp.capacity:
                raise ValueError("Camp is at full capacity")
            
            # Validate church exists
            church = Church.query.filter_by(id=registration_data['church_id'], camp_id=camp.id).first()
            if not church:
                raise ValueError("Invalid church selection")
            
            # Validate category exists and is allowed
            category = Category.query.filter_by(id=registration_data['category_id'], camp_id=camp.id).first()
            if not category:
            ...
            if link_token:
                ...
                if str(category.id) not in registration_link.allowed_categories:
                    raise ValueError("Selected category is not allowed for this registration link")
            ...
            new_registration = Registration(
                ...
                emergency_contact_name=registration_data['emergency_contact_name'].strip(),
                emergency_contact_phone=registration_data['emergency_contact_phone'].strip(),
                ...
                registration_link_id=registration_link.id if registration_link else None
            )
            ...
            current_app.logger.info(f"New registration created: {new_registration.surname} {new_registration.last_name} for camp {camp.name}")
            ...
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in create_registration: {str(e)}")
    def update_registration(self, registration_id: str, update_data: Dict[str, Any]) -> Optional[Registration]:
        ...
        try:
            ...
            if 'church_id' in update_data:
                church = Church.query.filter_by(id=update_data['church_id'], camp_id=registration.camp_id).first()
                if not church:
                    raise ValueError("Invalid church selection")
            
            # Validate category if being updated
            if 'category_id' in update_data:
                category = Category.query.filter_by(id=update_data['category_id'], camp_id=registration.camp_id).first()
                if not category:
                    raise ValueError("Invalid category selection")
            ...
            # Update fields
            updatable_fields = [
                'surname', 'middle_name', 'last_name', 'age', 'email', 'phone_number',
                'emergency_contact_name', 'emergency_contact_phone', 'church_id', 'category_id',
                'custom_field_responses', 'has_paid', 'has_checked_in'
            ]
            
            for field in updatable_fields:
                if field in update_data:
                    if field in ['surname', 'middle_name', 'last_name', 'emergency_contact_name'] and update_data[field] is not None:
                        ...
                        email = update_data[field].strip() if update_data[field] else None
            ...
            current_app.logger.info(f"Registration updated: {registration.surname} {registration.last_name}")
            return registration
            ...
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in update_registration: {str(e)}")
    ...
    def cancel_registration(self, registration_id: str) -> bool:
        ...
        try:
            ...
            current_app.logger.info(f"Registration cancelled: {registration.surname} {registration.last_name}")
            return True
            ...
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in cancel_registration: {str(e)}")
            raise Exception("Failed to cancel registration")
    
    def update_payment_status(self, registration_id: str, payment_data: Dict[str, Any]) -> Optional[Registration]:
        ...
        try:
            ...
            if has_paid:
                ...
                current_app.logger.info(f"Payment marked as unpaid for registration {registration_id}")
            ...
            db.session.rollback()
            current_app.logger.error(f"Database error in update_payment_status: {str(e)}")
            raise Exception("Failed to update payment status due to database error")
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in update_payment_status: {str(e)}")
            raise Exception("Failed to update payment status")
    
    def update_checkin_status(self, registration_id: str, checkin_data: Dict[str, Any]) -> Optional[Registration]:
        ...
        try:
            ...
            current_app.logger.info(f"Registration {status}: {registration.surname} {registration.last_name}")
            ...
            db.session.rollback()
            current_app.logger.error(f"Database error in update_checkin_status: {str(e)}")
            raise Exception("Failed to update check-in status due to database error")
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(f"Unexpected error in update_checkin_status: {str(e)}")
